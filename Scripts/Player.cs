...
public class Player : MonoBehaviour
{
    ...
    {
        // 0f,   // Speaker 1 (Front)
        // 60f,  // Speaker 2 (Front-Right)
        // 120f, // Speaker 3 (Right)
        // 180f, // Speaker 4 (Back)
        // 240f, // Speaker 5 (Back-Left)
        // 300f  // Speaker 6 (Left)

     30f,  // Speaker 1 (Front-Right)
     90f,  // Speaker 2 (Right)
     150f, // Speaker 3 (Back-Right)
     210f, // Speaker 4 (Back-Left)
     270f, // Speaker 5 (Left)
     330f  // Speaker 6 (Front-Left)
    };
...
// }
     ...
     void Update()
    {
        ...
        velVal = ((characterController.transform.position - previousPos) / Time.deltaTime).magnitude;
        previousPos = characterController.transform.position;
   ...
   private float GetAngleToObstacle(GameObject obstacle)
    ...
    /// for Arduino compatibility (kept for reference but unused)
 
    // private void SendHapticOutput(SpeakerOutput s1, SpeakerOutput s2)
    // {

    //     string udpMessage = $"{s1.SpeakerId:F3},{s1.Volume:F0},{s2.SpeakerId:F3},{s2.Volume:F0}";
    //     Debug.Log($"Legacy Arduino format: {udpMessage}");
    // }
}
...
// }
    ...
    //         string udpMessage = $"{zone}.0,{velVal:F1}";
    //         udpsender.strMessage = udpMessage;

    //         Debug.Log($"Player collided with {other.gameObject.name} → Zone {zone} | Angle: {angle:F1}°");

    //         // string timestamp = System.DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
    //         // string logLine = string.Format(
    //         //     "{0},{1:F2},{2:F2},{3:F2},{4},{5:F2},{6:F2},{7:F2},{8},{9:F1}\n",
    //         //     timestamp,
    ...
    //     void OnTriggerExit(Collider other)
    //     {
    //         udpsender.strMessage = $"10.0,{velVal:F1}";
    //         Debug.Log($"Player exitted {other.gameObject.name} ");
    //     }

    //     int GetZoneFromAngle(float angle)
    //     {//Different for every user, this is a general one
    //         if (angle >= 345f || angle < 15f)
    //             return 1;       // Front  Speaker 1
    //         else if (angle >= 15f && angle < 75f)
    //             return 12;      // Front-Right  Speakers 1+2
    //         else if (angle >= 75f && angle < 135f)
    //             return 23;      // Right  Speakers 2+3
    //         else if (angle >= 135f && angle < 195f)
    //             return 34;      // Back-Right  Speakers 3+4
    //         else if (angle >= 195f && angle < 255f)
    //             return 45;      // Back → Speakers 4+5
    //         else if (angle >= 255f && angle < 315f)
    //             return 56;      // Back-Left  Speakers 5+6
    //         else
    //             return 61;      // Left  Speakers 6+1
    //     }
    // }